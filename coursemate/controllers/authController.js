...
exports.login = async (req, res) => {
  ...
  try {
    ...
    // ✨ [핵심 수정] DB 컬럼명이 대문자(USER_ID)일 수도, 소문자(user_id)일 수도 있음!
    // 둘 다 체크해서 있는 값을 가져옵니다.
    const dbUserId = user.USER_ID || user.user_id;
    const dbEmail = user.EMAIL || user.email;

    // 로그인 성공 -> 토큰(JWT) 발급
    const token = jwt.sign(
      { userId: dbUserId, email: dbEmail }, // ✨ 확실한 ID 값 넣기
      JWT_SECRET,
      { expiresIn: '1d' }
    );
